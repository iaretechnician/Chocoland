...
if(!_check) exitWith {};
_objectscount = ["highscore" call PDB_databaseNameCompiler, "Count", "Count", "NUMBER"] call iniDB_read;
if(isNil "_objectscount") exitWith {};
for[{_i = 0}, {_i < _objectscount}, {_i = _i + 1}] do {
	_objSaveName = format["obj%1", _i];
	 _playerGUID = ["highscore" call PDB_databaseNameCompiler, _objSaveName, "playerGUID", "STRING"] call iniDB_read;
	        _class = ["highscore" call PDB_databaseNameCompiler, _objSaveName, "classname", "STRING"] call iniDB_read;
	_pos = ["highscore" call PDB_databaseNameCompiler, _objSaveName, "pos", "ARRAY"] call iniDB_read;
	_dir = ["highscore" call PDB_databaseNameCompiler, _objSaveName, "dir", "ARRAY"] call iniDB_read;
	_supplyleft = ["highscore" call PDB_databaseNameCompiler, _objSaveName, "supplyleft", "NUMBER"] call iniDB_read;
	_weapons = ["highscore" call PDB_databaseNameCompiler, _objSaveName, "weapons", "ARRAY"] call iniDB_read;
	_magazines = ["highscore" call PDB_databaseNameCompiler, _objSaveName, "magazines", "ARRAY"] call iniDB_read;
