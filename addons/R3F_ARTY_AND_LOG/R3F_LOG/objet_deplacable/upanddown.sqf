...
if (R3F_LOG_mutex_local_verrou) then {
	...
	_atth = - (_attcoord select 2);
     	switch (_upordown) do {
	case 1:
		{if ((_newheight - _playerheight) > 5.5) exitWith {player globalChat "You cant lift it so high up in the air.", "plain"; R3F_LOG_mutex_local_verrou = false;};
		_object attachTo [player, [ 0, (((boundingBox _object select 1 select 1) max (-(boundingBox _object select 0 select 1))) max ((boundingBox _object select 1 select 0) max (-(boundingBox _object select 0 select 0)))) + 1, _atth + 0.3]];};
	case 2:
		{_object attachTo [player, [ 0, (((boundingBox _object select 1 select 1) max (-(boundingBox _object select 0 select 1))) max ((boundingBox _object select 1 select 0) max (-(boundingBox _object select 0 select 0)))) + 1, _atth - 0.3]];};
	};
