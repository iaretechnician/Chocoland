...
//unlock
_objet addAction [("<t color=""#219eff"">" + STR_R3F_LOG_action_h + "</t>"), "addons\R3F\R3F_LOG\objet_deplacable\saver.sqf", _target, 2, false, true, "","R3F_LOG_objet_addAction == _target && str(_target getvariable'playerguid' select 0) == str (getplayeruid player) && isnil{_target getVariable puid}"];
_objet addAction [("<t color=""#219eff"">" + STR_R3F_LOG_action_g + "</t>"), "addons\R3F\R3F_LOG\objet_deplacable\loader.sqf",_target, 2, false, true, "","R3F_LOG_objet_addAction == _target && !isnil{_target getVariable puid}"];
