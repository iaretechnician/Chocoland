...
if(_type == "exit")exitwith{as0 = _man addAction [localize "STR_MENUASM","client\systems\FunStore\adds\airSup\addMenu.sqf",[], 5, false, false, "", "_target == player"]};
...
if((_spwn select 2) < 0)then{_spwnz = 0}else{_spwnz = _spwn select 2};

_plane = createVehicle [airSupPlane, [_spwn select 0,_spwn select 1,(_spwnz)+200], [], 0, "FLY"];
...
switch (_type) do 
{
	case "jdam":{
		...
		if (alive _plane) then {
			_b1 = airSupBomb createvehicle [getPos _plane select 0,getPos _plane select 1,(getPos _plane select 2)-10];
...
if(true)exitWith{as0 = _man addAction [localize "STR_MENUASM","client\systems\FunStore\adds\airSup\addMenu.sqf",[], 5, false, false, "", "_target == player"]};