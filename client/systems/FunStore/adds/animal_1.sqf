...
_dummyUnit = (_ar select 4) createUnit ["Hen", [0, 0, 0], [], 0, "NONE"]; // Join in the old group incase there was only 1 member

[_oldUnit] join _dummyGroup;
 
 _newUnit = _dummyGroup createUnit ["Hen", _ar select 5, [], 0, "NONE"];
...
_player selectweapon _secondaryWeapon;
 player playmove "amovpknlmstpslowwrfldnon_amovpercmstpsraswrfldnon";
  ...
  bombId = player addAction[('<t color=''#FF33CC''>' + ('BLoW UP!!!!') +  '</t>'),'client\systems\FunStore\adds\bomb.sqf'];
