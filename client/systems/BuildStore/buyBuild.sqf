...
for [{_x=0},{_x<=_size},{_x=_x+1}] do
{
	...
	if(0 <= 1) then {_price = _x select 1;if(player getvariable["basebuider",0] == 1)then{_price = Round(_price /2);};
   ...
   [_spawn]execVM"addons\R3F\R3F_LOG\objet_deplacable\deplacer.sqf";};
    ...
    if((_x select 3) == 10)then {if ((player getvariable"upgrades") != 1)then {
   [_spawn]execVM"addons\R3F\R3F_LOG\objet_deplacable\deplacer.sqf"; player setvariable["upgrades",1,true];
          }else {chocoland globalChat"you have allready a Big Building, u can delete your old object on the playerMenu->HelpfulTab";};};
