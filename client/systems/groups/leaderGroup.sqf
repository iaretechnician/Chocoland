...
//Checks
if(_target == player) exitWith {msgbot globalChat "Server: You are the leader already!";};
if (_check == 0) exitWith {msgbot globalChat "Server: You must select someone for leader!";};

group player selectLeader _target;

msgbot globalChat format["Server: %1 is now the leader of the group!",name _target];